...
class Wrapper(object):
    ...
    def __iter__(self):
        if (
            isinstance(self._wrapped_object, list)
            or isinstance(self._wrapped_object, tuple)
            or isinstance(self._wrapped_object, dict)
        ):
            yield from self._wrapped_object

    def __len__(self):
        return (
            len(self._wrapped_object)
            if isinstance(self._wrapped_object, list)
            or isinstance(self._wrapped_object, tuple)
            or isinstance(self._wrapped_object, dict)
            else 0
...
def inspect(object_: Wrapper, show_values: bool = True, show_types: bool = False, filter_by: str = None) -> list[str]:
...
def print_inspect(object_: Wrapper, show_values: bool = True, show_types: bool = False, filter_by: str = None):
    ...
    Args:
                    ...
                    filter_by (str, optional): a regex to filter the paths
    """
    for path in inspect(object_, show_values=show_values, show_types=show_types, filter_by=filter_by):
        print(path)
